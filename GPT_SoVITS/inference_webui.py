...
cnhubert_base_path = "/home/mlb/Documents/PycharmProject/ai-collect/develop/GPT-SoVITS/GPT_SoVITS/pretrained_models/wavlm-plus"
bert_path = "/home/mlb/Documents/PycharmProject/ai-collect/develop/GPT-SoVITS/GPT_SoVITS/pretrained_models/USER-bge-m3"
...
v3v4set = {"v3", "v4"}
vq_model = hps = if_lora_v3 = None

def change_sovits_weights(sovits_path, prompt_language=None, text_language=None):
    print(f"LOADING SOVITS {sovits_path}")
    if "！" in sovits_path or "!" in sovits_path:
        sovits_path = name2sovits_path[sovits_path]
    global vq_model, hps, version, model_version, dict_language, if_lora_v3
    prompt_text_update = prompt_language_update = text_update = text_language_update = visible_sample_steps = visible_inp_refs = None
    ...
    yield (
        {"__type__": "update", "choices": list(dict_language.keys())},
        {"__type__": "update", "choices": list(dict_language.keys())},
        prompt_text_update if prompt_text_update else None,
        prompt_language_update if prompt_language_update else None,
        text_update if text_update else None,
        text_language_update if text_language_update else None,
        {
            "__type__": "update",
            "visible": visible_sample_steps,
            "value": 32 if model_version == "v3" else 8,
...
def get_bert_feature_rus(bert_model, tokenizer, norma_text, word_list, phoneme_count_per_word, masked_string):
    """
...
def get_ru_data(text):
    from transformers import AutoModel, AutoTokenizer
    tokenizer = AutoTokenizer.from_pretrained("/home/mlb/Documents/PycharmProject/ai-collect/develop/GPT-SoVITS/GPT_SoVITS/pretrained_models/USER-bge-m3")
    bert_model = AutoModel.from_pretrained("/home/mlb/Documents/PycharmProject/ai-collect/develop/GPT-SoVITS/GPT_SoVITS/pretrained_models/USER-bge-m3")
    if is_half == True:
        bert_model = bert_model.half().to(device)
    else:
        bert_model = bert_model.to(device)
    bert_model.eval()
    print(f"LOADED BERT /home/mlb/Documents/PycharmProject/ai-collect/develop/GPT-SoVITS/GPT_SoVITS/pretrained_models/USER-bge-m3")

    phones_return, words, word2ph, norm_text, string_mask = clean_text(text, "ru", "v2")
    print(f"==get_ru_data: input text: {text}\nphones: {phones_return}\nwords: {words}\nword2ph: {word2ph}\nnorm_text: {norm_text}\nstring_mask: {string_mask}")
    bert_feature = get_bert_feature_rus(bert_model, tokenizer, norm_text, words, word2ph, string_mask)

    phones = cleaned_text_to_sequence(phones_return, "v2")
    print(f"==cleaned_text_to_sequence: phones: {phones}")
    return phones, bert_feature.to(dtype), norm_text

def get_phones_and_bert(text, language, version, final=False):
    print(f"-- get_phones_and_bert text {text} lang {language} v {version}")
    if language == "ru":
        return get_ru_data(text)
    ...
    elif language in {"zh", "ja", "ko", "yue", "auto", "auto_yue", "ru"}:
        textlist = []
        langlist = []
        if language == "auto":
            ...
            for tmp in LangSegmenter.getTexts(text):
                if langlist:
                    if (tmp["lang"] == "en" and langlist[-1] == "en") or (tmp["lang"] != "en" and langlist[-1] != "en"):
                        textlist[-1] += tmp["text"]
                        continue
                if tmp["lang"] == "en" and language != "ru":
                    langlist.append(tmp["lang"])
                else:
                    # 因无法区别中日韩文汉字,以用户输入为准
...
def get_tts_wav(
    ...
    if not ref_free:
        with torch.no_grad():
            ...
            #     wav16k = torchaudio.transforms.Resample(sr, 16000).to(device)(wav16k)
            # else:
            #     wav16k = wav16k.to(device)
            #     if wav16k.shape[0] == 2:
            #         wav16k = wav16k.mean(0)
    ...
    if not ref_free:
        phones1, bert1, norm_text1 = get_phones_and_bert(prompt_text, "ru", version)
    ...
    for i_text, text in enumerate(texts):
        ...
        phones2, bert2, norm_text2 = get_phones_and_bert(text, "ru", version)
        print("== TAGERT TEXT: norm [{norm_text2}]\n{phones2}\n{}")
        if not ref_free:
