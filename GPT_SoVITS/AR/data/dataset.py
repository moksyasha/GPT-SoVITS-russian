...
class Text2SemanticDataset(Dataset):
    ...
    def __init__(
        ...
        print(f"PATH phoneme_path: {phoneme_path}")
        print(f"PATH semantic_path: {semantic_path}")
        for id, line in enumerate(lines):
            tmp = line.split("\t")
            if id < 1: print(f"1TMP {tmp}")
            if len(tmp) != 4:
                continue

            filename = os.path.basename(tmp[0])
            self.phoneme_data[filename] = [tmp[1], tmp[2], tmp[3]]
        ...
        #         continue
        #     filename = os.path.basename(tmp[0])
        #     self.phoneme_data[filename] = [tmp[1]]
