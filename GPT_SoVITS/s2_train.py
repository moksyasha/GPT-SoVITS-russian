...
# from sv import SV
# sv_model = SV(device='cuda:0', is_half=True)
...
def run(rank, n_gpus, hps):
    ...
    train_dataset = TextAudioSpeakerLoader(hps.data, hps.data.sampling_rate)#,version=hps.model.version)
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        ...
        if torch.cuda.is_available():
            ...
            if hps.model.version in {"v2Pro", "v2ProPlus"}:
                ...
                sv_emb = sv_emb.float()
                # Создаем "пустой" sv_emb правильной формы и типа
                # batch_size = ssl.size(0)
                # in_features = 20480 # Из лога ошибки
                # sv_emb = torch.zeros(batch_size, in_features).cuda(rank, non_blocking=True)
                # sv_emb = sv_emb.float()
        else:
        ...
        if rank == 0:
            if global_step % hps.train.log_interval == 0:
                ...
                losses = [f"loss_disc {loss_disc.item()}", f"\n-loss_gen {loss_gen.item()}", f"\n-loss_fm {loss_fm.item()}", f"\n-loss_mel {loss_mel.item()}",\
                           f"\n-kl_ssl {kl_ssl.item()}", f"\n-loss_kl {loss_kl.item()}"]
                logger.info(
                ...
                logger.info([x for x in losses] + [global_step, lr])
